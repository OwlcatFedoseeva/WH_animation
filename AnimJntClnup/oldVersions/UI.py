...
DIRECTORY = os.path.join(USERAPPDIR, 'processedAnimation')
...
class batch_cleanup_UI(QtWidgets.QDialog):
    ...
    def btn_batch_process_clicked(self):
        if not self.anim_folder_path:
            cmds.warning("Please set the animation folder path.")
            return

        fbx_files = [f for f in os.listdir(self.anim_folder_path) if f.endswith('.fbx')]
        if not fbx_files:
            cmds.warning("No FBX files found in the specified folder.")
            return

        for fbx_file in fbx_files:
            full_path = os.path.join(self.anim_folder_path, fbx_file)
            self.open_fbx_file(full_path)

    def open_fbx_file(self, file_path):
        # Load the FBX export preset
        load_fbx_export_preset(presetPathName)

        # Import the FBX file
        cmds.file(file_path, i=True, type="FBX", ignoreVersion=True, ra=True, mergeNamespacesOnClash=False, namespace=":")

        # Perform additional operations if needed
        file_short_name = os.path.basename(file_path)
        ...
        # Save the file to overwrite it in the same folder
        cmds.select('ParentForExportDelete', hierarchy=True)
        nameExport = os.path.join(self.export_dir, file_name + '.fbx')
        cmds.FBXExport(s=True, f=nameExport, force=True, options=True)
